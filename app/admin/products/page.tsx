  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map(p => (
          <div key={p.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden group hover:shadow-md transition">
            <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
              ...
              {p.badge && (
                <span className="absolute top-2 left-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-full">{p.badge}</span>
              )}
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                <button onClick={() => setEditing({ ...p })}
                  className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition">
                  <Pencil className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(p.id)}
                  className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-700 hover:bg-red-50 hover:text-red-600 transition">
            ...
            <div className="p-4">
              ...
              <h3 className="font-medium text-slate-900 mt-1">{p.name}</h3>
              <p className="text-sm text-slate-500 mt-1 line-clamp-2">{p.shortDescription}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-lg font-bold text-slate-900">${p.price}</span>
                {p.compareAt && <span className="text-sm text-slate-400 line-through">${p.compareAt}</span>}
              </div>
            </div>
          </div>
        ))}
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  <button onClick={() => setPickerOpen(true)}
                    className="w-24 h-24 rounded-lg bg-slate-100 overflow-hidden border-2 border-dashed border-slate-300 hover:border-blue-500 transition flex items-center justify-center">
                    {editing.imageSrc ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={editing.category} onChange={e => setEditing({ ...editing, category: e.target.value })}
                    className="w-full mt-1 px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
              ...
              <div>
                ...
                <textarea value={editing.shortDescription} onChange={e => setEditing({ ...editing, shortDescription: e.target.value })}
                  rows={2} className="w-full mt-1 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none" />
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700">Compare At ($)</label>
              ...
              <div>
                ...
                <textarea value={editing.features.join('\n')}
                  ...
                  rows={3} className="w-full mt-1 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none" />
              </div>
            </div>
